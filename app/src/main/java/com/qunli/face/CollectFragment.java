...
import com.bcq.oklib.utils.ScreenUtil;
import com.qunli.face.widget.BrightnessTools;
...
public class CollectFragment extends BaseFragment {
    ...
    public void init() {
        ...
        spoint = ScreenUtil.getScreenPoint();
        initView();
    ...
    private void initView() {
        previewView = getView(R.id.preview_view);
        mTextureView = getView(R.id.texture_view);
        mTextureView.setOpaque(false);
        mRecyclerview = getView(R.id.recyclerview);
        // 不需要屏幕自动变黑。
        mTextureView.setKeepScreenOn(true);
        final CameraImageSource cameraImageSource = new CameraImageSource(mActivity);
     ...
     */
    private void start() {
        RectF newDetectedRect = new RectF(0, 0, spoint.x, spoint.y);
    ...
    private static class InnerHandler extends Handler {
        private WeakReference<CollectFragment> mWeakReference;

        public InnerHandler(CollectFragment activity) {
            super();
        ...
        public void handleMessage(Message msg) {
            ...
            CollectFragment activity = mWeakReference.get();
            if (activity == null) {
                return;
            }
            if (msg == null) {
...
//            }
    ...
    private void initRecy() {
        mRecyAdapter = new RecyAdapter(mActivity);
        mLayoutManager = new LinearLayoutManager(mActivity, LinearLayoutManager.HORIZONTAL, true);
